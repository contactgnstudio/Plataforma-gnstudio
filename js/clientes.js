// ============================================================
// js/clientes.js — GN Studio OS + Supabase  v2.5
// CRM: tabla de clientes + búsqueda DGI Panamá integrada
// ============================================================
...
async function obtenerProyectos() {
  var key = STORAGE_KEYS.PROYECTOS || 'proyectos';
...
async function obtenerConteoProyectosPorCliente() {
  ...
  (proyectos || []).forEach(function(p) {
    var cid = String(p.cliente_id || p.clienteId || '').trim();
    if (!cid) return;
...
async function guardarCliente(event) {
  ...
  if (editingId) {
    ...
    if (!okUpdate) { alert('No se pudo actualizar el cliente.'); return false; }
    cerrarModalCliente();
    await renderClientes();
    await actualizarSelectClientes();
    if (window.showToast) window.showToast({ type:'success', title:'Cliente actualizado', message: payload.nombre + ' actualizado.' });
    else alert('Cliente actualizado exitosamente.');
    return false;
  }

  var resultado = await addItem(STORAGE_KEYS.CLIENTES, payload);
  if (!resultado) { alert('No se pudo guardar el cliente en Supabase.'); return false; }

  cerrarModalCliente();
  await renderClientes();
  await actualizarSelectClientes();
  if (window.showToast) window.showToast({ type:'success', title:'Cliente guardado', message: payload.nombre + ' agregado.' });
  else alert('Cliente guardado exitosamente.');
  return false;
}
...
async function renderClientes() {
  ...
  if (!Array.isArray(clientes) || clientes.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty-state">No hay clientes registrados. Usa el botón <strong>Buscar en DGI</strong> para añadir clientes.</td></tr>';
    return;
  }

  var html = '';
  clientes.forEach(function(c) {
    var rucCompleto = c.ruc ? (c.ruc + (c.dv ? '-' + c.dv : '')) : '-';
    var telefonoPrincipal = c.telefono || c.telefono_secundario || '-';
    var clienteId = String(c.id || '').trim();
    var proyectosCount = clienteId && mapaProyectos[clienteId] ? mapaProyectos[clienteId] : 0;
    var fuente = c.fuente === 'dgi'
      ? '<span style="font-size:10px;padding:2px 6px;background:rgba(197,162,83,0.15);color:#C5A253;border-radius:99px;font-weight:700;">DGI</span>'
      : '';

    html += '<tr>';
    html += '<td>' + escapeHtmlCliente(c.nombre || c.nombre_comercial || 'Sin nombre') + ' ' + fuente + '</td>';
    html += '<td>' + escapeHtmlCliente(rucCompleto) + '</td>';
    html += '<td>' + escapeHtmlCliente(c.correo || '-') + '</td>';
    html += '<td>' + escapeHtmlCliente(telefonoPrincipal) + '</td>';
    html += '<td style="text-align:center;">' + proyectosCount + '</td>';
    // Tipo contribuyente
    html += '<td>' + escapeHtmlCliente(c.tipo_contribuyente || '-') + '</td>';
    html += '<td>';
...
async function actualizarSelectClientes() {
  ...
  if (selectCotizacion) selects.push(selectCotizacion);
  ...
  selects.forEach(function(sel) {
    ...
    clientes.forEach(function(c) {
      ...
      opt.value = c.id;
      opt.textContent = (c.nombre || c.nombre_comercial || 'Sin nombre') + (c.ruc ? ' (' + c.ruc + ')' : '');
      sel.appendChild(opt);
    });
...
// ============================================================
// Modal cliente — Nuevo / Editar manual
// ============================================================
function abrirModalCliente(id) {
      ...
      '<form id="form-cliente" onsubmit="return guardarCliente(event);">',
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">',
          ...
          '<div style="grid-column:1/-1;">',
            ...
            '<textarea id="cli-notas" rows="3" placeholder="Notas internas del cliente" style="width:100%;padding:10px;background:#0D1611;border:1px solid rgba(18,53,36,0.25);border-radius:8px;color:#F0F0F5;box-sizing:border-box;resize:vertical;"></textarea>',
          '</div>',
        '</div>',
        '<div style="display:flex;gap:12px;justify-content:flex-end;margin-top:24px;">',
          ...
          '<button type="submit" style="padding:10px 20px;background:#C5A253;border:none;border-radius:8px;color:#111;font-weight:700;cursor:pointer;">Guardar Cliente</button>',
        '</div>',
      '</form>',
    '</div>'
  ].join('');
  ...
  modal.addEventListener('click', function(e) { if (e.target === modal) cerrarModalCliente(); });
}
